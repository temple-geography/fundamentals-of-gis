...
> **IMPORTANT NOTE:** *If you change computers* (for example, if you do not complete the lab in one sitting), when you open DB Manager you will not see your `SuperfundProject.sqlite` database. QGIS registers a specific path to the SpatiaLite database.^[This is similar to the Connect To Folder concept in ArcGIS.] In order to connect to the database, do the following:
>
