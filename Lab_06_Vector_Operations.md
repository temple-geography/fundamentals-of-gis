...
3.  Click Yes to confirm you want to edit this field. In the Calculate Geometry dialog box make sure the Units: reads Meters \[m\]. Click OK. You should see the new length values appear in the new field.
4.  Add the Zipcodes Poly shapefile to ArcMap and open its attribute table. Find the SHAPE\_Area field. This field encodes the area of each zip code in square feet.
5.  Create a new field called Area\_m and calculate its value to be the area in square meters. Follow the same procedures as for the Bike Network but choose to calculate Area in square meters.
...
8.  In some cases, it is useful to have an individual record for each feature. To make this conversion, where each polygon is represented as a single record in the attribute table, go to ArcToolbox?Data Management Tools→Features→Multipart to Singlepart and input schools\_buf1k and name the output schools\_buf1k\_single. Open the output table from the resulting shapefile and confirm that it has multiple records—one for each feature.
...
Occasionally it is useful analytically to break those discrete objects apart. For instance, if you needed to know the area of each of the features, because they are encoded as a single object. The multipart to singlepart tool creates a new feature for each non-contiguous feature. So each "blob" you see in the intersection file would become a new object in the resulting file, with a unique record in the resulting attribute table (allowing you to add a field and calculate the area of each). The geography of the file will not change - only the attribute table. 1. Select the part of the union table that is within the school buffer, but outside of the bike network buffer. 0 for the school buffer column and -1 for the bike network buffer. (You just created this union file with the bike network buffer and the school buffer) 2. Right click on the union .shp file in the table of conents and click on data and export data and create a new shape file out of the union selection. You can call it School\_not\_Network 3. Press control + F to bring up the search dialogue. Type in 'multipart' Choose the tool multipart to singlepart. 4. Drag in School\_not\_work as the input. Name the new file mpart\_school\_not\_work. Press ok. 5. Inspect the resulting file. The map features of mpart\_school\_not\_work look exactly like School\_not\_work. Inspect the attribute table, however, and you'll see that the new file has a different record for each discrete feature from the school\_not\_work file.
